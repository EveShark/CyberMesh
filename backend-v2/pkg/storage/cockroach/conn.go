...
func NewConnection(ctx context.Context, cfg *ConnectionConfig) (*sql.DB, error) {
	...
	// Add application_name to DSN to attribute writers in CockroachDB (crdb_internal).
	// Best-effort; never fails connection setup.
	setApplicationName(cfg)
...
func setApplicationName(cfg *ConnectionConfig) {
	if cfg == nil || cfg.ConfigManager == nil {
		return
	}
	...
	appName := cfg.ConfigManager.GetString("DB_APPLICATION_NAME", "")
	if appName == "" {
		nodeID := cfg.ConfigManager.GetString("NODE_ID", "")
		pod := cfg.ConfigManager.GetString("POD_NAME", "")
		if pod == "" {
			pod = cfg.ConfigManager.GetString("HOSTNAME", "")
		}
		appName = "cybermesh"
		if nodeID != "" {
			appName = fmt.Sprintf("%s-validator-%s", appName, nodeID)
		} else if pod != "" {
			appName = fmt.Sprintf("%s-%s", appName, pod)
		}
	}

	updated, err := setDSNQueryParam(cfg.DSN, "application_name", appName)
	if err != nil {
		return
	}
	cfg.DSN = updated
...
func setDSNQueryParam(dsn string, key string, value string) (string, error) {
	u, err := url.Parse(dsn)
	if err != nil {
		return dsn, err
	}
...
func openConnectionWithTimeout(ctx context.Context, cfg *ConnectionConfig) (*sql.DB, error) {
	...
	if err := db.PingContext(timeoutCtx); err != nil {
		db.Close()
		// SECURITY: Never log the DSN. The ping error itself should not contain credentials;
		// include it to surface actionable causes (TLS, auth, DNS, etc.).
