...
var ErrMissingJustifyQC = errors.New("proposal missing required JustifyQC")
...
func (v *Validator) ValidateProposal(ctx context.Context, p *Proposal) error {
	...
	if p.JustifyQC != nil {
		...
		// Genesis (height 1) is allowed to omit JustifyQC.
		// If we get here with height > 1, the node is not bootstrapped correctly.
		if v.logger != nil {
			v.logger.WarnContext(ctx, "proposal rejected: missing JustifyQC",
				"view", p.View,
				"height", p.Height,
				"proposer_id", fmt.Sprintf("%x", p.ProposerID[:8]),
				"parent_hash", fmt.Sprintf("%x", p.ParentHash[:8]),
		...
		return fmt.Errorf("%w (height=%d)", ErrMissingJustifyQC, p.Height)
	}
