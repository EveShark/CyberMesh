...
type HeartbeatManager struct {
	...
	logLast map[ValidatorID]time.Time
...
func NewHeartbeatManager(
	...
	return &HeartbeatManager{
		...
		lastProposal:  time.Time{},
		logLast:       make(map[ValidatorID]time.Time),
		stopCh:        make(chan struct{}),
		callbacks:     callbacks,
	}
}

func (hm *HeartbeatManager) shouldLogLocked(key ValidatorID, every time.Duration) bool {
	now := time.Now()
	if last, ok := hm.logLast[key]; ok && now.Sub(last) < every {
		return false
	}
	hm.logLast[key] = now
	return true
...
func (hm *HeartbeatManager) runChecker(ctx context.Context) {
	...
	for {
		...
		case <-timer.C:
			...
			default:
				// Reset timer
				timer.Reset(hm.config.MaxIdleTime)
			}
		}
	}
}
...
func (hm *HeartbeatManager) OnHeartbeat(ctx context.Context, hbMsg *HeartbeatMsg) error {
	...
	currentHeight := hm.pacemaker.GetCurrentHeight()

	const logEvery = 30 * time.Second
	viewDelta := int64(hbMsg.View) - int64(currentView)
	heightDelta := int64(hbMsg.Height) - int64(currentHeight)

	// Validate heartbeat is for current view
	if hbMsg.View != currentView {
		if hm.logger != nil && hm.shouldLogLocked(hbMsg.LeaderID, logEvery) {
			hm.logger.WarnContext(ctx, "heartbeat view mismatch",
				"leader", fmt.Sprintf("%x", hbMsg.LeaderID[:8]),
				"leader_view", hbMsg.View,
				"local_view", currentView,
				"view_delta", viewDelta,
				"leader_height", hbMsg.Height,
