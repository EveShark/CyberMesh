...
func (s *Server) writePrometheusMetrics(w io.Writer) {
	...
	if s.engine != nil {
		...
		fmt.Fprintf(w, "# HELP consensus_invalid_blocks_total Total committed-candidate blocks rejected by the application/state layer as deterministically invalid.\n")
		fmt.Fprintf(w, "# TYPE consensus_invalid_blocks_total counter\n")
		fmt.Fprintf(w, "consensus_invalid_blocks_total %d\n", snap.InvalidBlocks)
	}
