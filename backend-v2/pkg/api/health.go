...
func (s *Server) runConsensusCheck(ctx context.Context) (ReadinessCheckResult, interface{}, string) {
	...
	active := s.engine.IsConsensusActive()
	...
	details := map[string]interface{}{
		...
		"consensus_metadata_repair_last_unix": drift.LastRepairUnix,
		"consensus_metadata_repair_error":     drift.LastError,
	}

	// Best-effort drift snapshot direct from DB (same DB used by the node).
	if s.storage != nil {
		if provider, ok := s.storage.(interface{ GetDB() *sql.DB }); ok {
			db := provider.GetDB()
			if db != nil {
				var blocksHeight sql.NullInt64
				if err := db.QueryRowContext(ctx, `SELECT MAX(height) FROM blocks`).Scan(&blocksHeight); err == nil {
					durable := int64(0)
					if blocksHeight.Valid {
						durable = blocksHeight.Int64
					}
					details["consensus_durable_height"] = durable
					...
					if err := db.QueryRowContext(ctx, `SELECT height FROM consensus_metadata WHERE key = 'last_committed'`).Scan(&metaHeight); err == nil {
						meta := int64(0)
						if metaHeight.Valid {
							meta = metaHeight.Int64
						}
						details["consensus_metadata_height"] = meta
						delta := durable - meta
						if delta < 0 {
							delta = -delta
						}
						details["consensus_height_drift"] = delta
					}
				}
			}
		}
	...
	var phase string
	if drift.LastError != "" {
		details["consensus_metadata_drift_unhealthy"] = true
		message = fmt.Sprintf("consensus metadata repair failing: %s", drift.LastError)
		res := buildCheckResult(start, "not ready", errors.New(message))
		return res, details, "degraded"
	}
	...
	case active && activation.HasQuorum:
		phase = "active"
