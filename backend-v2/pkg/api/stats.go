...
type aiDetectionStatsPayload struct {
	...
	DetectionLoop *struct {
		...
		Metrics                   *struct {
			...
			LastIterationTime     float64                `json:"last_iteration_time"`
			LoopIterations        uint64                 `json:"loop_iterations"`
			AvgLatencyMs          float64                `json:"avg_latency_ms"`
			LastLatencyMs         float64                `json:"last_latency_ms"`
			PolicyCandidates      uint64                 `json:"policy_candidates"`
			PolicyPublished       uint64                 `json:"policy_published"`
			PolicyFailed          uint64                 `json:"policy_failed"`
			PolicySkipped         uint64                 `json:"policy_skipped"`
			PolicySkipReasons     map[string]interface{} `json:"policy_skip_reasons"`
		} `json:"metrics"`
	} `json:"detection_loop"`
	Derived *struct {
...
func (s *Server) buildStatsResponse(ctx context.Context) StatsResponse {
	...
	// Fetch all stats concurrently using errgroup
	// errgroup automatically cancels context on first error and waits for all goroutines
	g, gctx := errgroup.WithContext(ctx)

	g.Go(func() error {
		chainStats = s.getChainStats(gctx)
		return nil
	})

	g.Go(func() error {
		consensusStats = s.getConsensusStats(gctx)
		return nil
	})

	g.Go(func() error {
		mempoolStats = s.getMempoolStats(gctx)
		return nil
	})

	g.Go(func() error {
		networkStats = s.getNetworkStats(gctx)
		return nil
	})

	g.Go(func() error {
		kafkaStats = s.getKafkaStats(gctx)
		return nil
	})

	g.Go(func() error {
		storageStats = s.getStorageStats(gctx)
		return nil
	})

	g.Go(func() error {
		redisStats = s.getRedisStats(gctx)
		return nil
	})

	g.Go(func() error {
		aiStats = s.getAIStats(gctx)
		return nil
	})
	...
	// Build response from concurrent results
...
func (s *Server) getConsensusStats(ctx context.Context) *ConsensusStats {
	...
	storageStats := s.engine.GetPBFTStorageStats()
	stats.LastCommittedHeight = storageStats.LastCommittedHeight
	stats.ReplayWindowSize = storageStats.ReplayWindowSize
	stats.RestoreDroppedProposalsStale = storageStats.RestoreDroppedProposalsStale
	stats.RestoreDroppedProposalsAhead = storageStats.RestoreDroppedProposalsAhead
	stats.RestoreDroppedVotesStale = storageStats.RestoreDroppedVotesStale
	stats.RestoreDroppedVotesAhead = storageStats.RestoreDroppedVotesAhead
	stats.RestoreDroppedQCsStale = storageStats.RestoreDroppedQCsStale
	stats.RestoreDroppedQCsAhead = storageStats.RestoreDroppedQCsAhead
	stats.RestoreDroppedEvidenceStale = storageStats.RestoreDroppedEvidenceStale
	stats.RestoreDroppedEvidenceAhead = storageStats.RestoreDroppedEvidenceAhead

	drift := consensusapi.PrivateGetDriftStatus(s.engine)
	stats.MetadataRepairLastUnix = drift.LastRepairUnix
	stats.MetadataRepairError = drift.LastError
	...
	if s.storage != nil {
		if provider, ok := s.storage.(interface{ GetDB() *sql.DB }); ok {
			...
			if db != nil {
				...
				if err := db.QueryRowContext(ctx, `SELECT height FROM consensus_metadata WHERE key = 'last_committed'`).Scan(&metaHeight); err == nil {
					if metaHeight.Valid {
						stats.MetadataHeight = metaHeight.Int64
						delta := int64(stats.LastCommittedHeight) - metaHeight.Int64
						if delta < 0 {
							delta = -delta
						}
						stats.HeightDrift = delta
					}
				}
			}
		}
