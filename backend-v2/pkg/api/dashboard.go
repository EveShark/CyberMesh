...
func (s *Server) handleDashboardOverview(w http.ResponseWriter, r *http.Request) {
	...
	var (
		...
		consensusMetrics  consapi.MetricsSnapshot
	...
	// Phase 1: Fetch independent data concurrently using errgroup
	// errgroup automatically cancels context on first error and waits for all goroutines
	g, gctx := errgroup.WithContext(ctx)

	// Stats - foundational data, but can run in parallel with non-dependent fetches
	g.Go(func() error {
		stats = s.buildStatsResponse(gctx)
		return nil
	})

	// Network overview - independent
	g.Go(func() error {
		if s.engine != nil {
	...
	// Consensus overview and metrics - independent
	g.Go(func() error {
		if s.engine != nil {
	...
	// Threats section - calls AI service, independent
	g.Go(func() error {
		threatsSection = s.buildDashboardThreats(gctx)
		return nil
	})

	// AI section - calls AI service, independent
	g.Go(func() error {
		aiSection = s.buildDashboardAI(gctx)
		return nil
	})

	// Validators section - independent
	g.Go(func() error {
		validatorSection = s.buildDashboardValidators(gctx)
		return nil
	})

	// Readiness check - independent
	g.Go(func() error {
		readiness, _ = s.buildReadinessResponse(gctx)
		return nil
	})

	// Wait for all Phase 1 goroutines to complete
	if err := g.Wait(); err != nil {
		// All goroutines return nil, but log unexpected errors
		if s.logger != nil {
			s.logger.WarnContext(ctx, "dashboard concurrent fetch error", utils.ZapError(err))
		}
	}

	// Phase 2: Fetch dependent data (requires stats from Phase 1)
	blocksSection := s.buildDashboardBlocks(ctx, stats, consensusMetrics)
	ledgerSection := s.buildDashboardLedger(ctx, stats)

	// Build derived metrics (CPU-bound, fast)
