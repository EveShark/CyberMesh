...
function getStatusClass(status?: string) {
  ...
  if (tone === "healthy") {
    return { bg: "var(--status-healthy)", text: "var(--status-healthy)" }
  }
  if (tone === "warning") {
    return { bg: "var(--status-warning)", text: "var(--status-warning)" }
  }
  return { bg: "var(--status-critical)", text: "var(--status-critical)" }
}
...
export default function NetworkGraph({ nodes, edges, leader, leaderId, isLoading, error }: NetworkGraphProps) {
  ...
  const validators = useMemo(() => {
    return nodes.slice(0, 5).map((node) => {
      const isLeader = leaderId ? node.id === leaderId : leader ? Boolean(node.name?.includes(leader) || node.id.includes(leader)) : false
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center min-h-[300px] md:min-h-[500px]">
        <div className="text-center space-y-3">
          <AlertCircle className="h-8 w-8 mx-auto text-[color:var(--status-critical)]" />
          <p className="text-sm text-muted-foreground">Failed to load network topology</p>
          <p className="text-xs text-[color:var(--status-critical)]">{error.message}</p>
        </div>
      </div>
    )
  }
          ...
          {validators.map((node) => {
            ...
            return (
              <button
                ...
                className={`text-left rounded-2xl border transition-all duration-300 backdrop-blur-xl p-4 ${
                  isSelected ? "border-primary/50 bg-background/80 shadow-xl" : "border-border/30 bg-background/60 shadow-lg"
                }`}
              >
                ...
                {isSelected && (
                  <div className="mt-3 pt-3 border-t border-border/30 text-xs text-muted-foreground grid grid-cols-2 gap-2">
                    <div>
  ...
  return (
    <div className="space-y-6">
      <div ref={containerRef} className="relative mx-auto w-full max-w-4xl" style={{ minHeight: 520 }}>
        <canvas ref={canvasRef} className="absolute inset-0 pointer-events-none" />

        <div className="absolute inset-0 flex items-center justify-center">
          <div className="relative">
            <div
              className={`flex h-24 w-24 items-center justify-center rounded-full border backdrop-blur-xl transition-colors duration-500 ${
                allHealthyConnected ? "border-primary/60 bg-primary/15 shadow-[0_0_35px_rgba(59,130,246,0.35)]" : "border-border/40 bg-primary/10"
              }`}
            >
            ...
            <div
              className={`absolute inset-0 rounded-full border transition-colors duration-500 ${
                allHealthyConnected ? "border-primary/50 animate-pulse" : "border-primary/20"
              }`}
            />
            {allHealthyConnected && (
        ...
        {layout.map((node) => {
          ...
          return (
            <button
              ...
              className={`group absolute flex w-40 sm:w-44 -translate-x-1/2 -translate-y-1/2 flex-col rounded-2xl border backdrop-blur-xl transition-transform duration-300 ${
                isSelected
            ...
            >
              {node.isLeader && (
                <span className="absolute -top-6 left-1/2 -translate-x-1/2 animate-bounce">
                  <Crown className="h-5 w-5 text-[color:var(--status-warning)]" />
                </span>
              )}
              <span
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <h3 className="text-lg font-semibold text-foreground flex items-center gap-2">
                {selectedNode.isLeader && <Crown className="h-5 w-5 text-[color:var(--status-warning)]" />}
                {selectedNode.name}
              </h3>
      ...
      <div className="rounded-2xl border border-border/30 bg-background/70 p-4 backdrop-blur-xl">
        <h3 className="mb-4 flex items-center gap-2 text-sm font-semibold text-foreground">
          <Activity className="h-4 w-4" /> Legend
        </h3>
        <div className="grid grid-cols-1 gap-4 text-xs text-muted-foreground md:grid-cols-2">
          <div className="space-y-3">
            ...
            {[
              { label: "Healthy", color: "var(--status-healthy)", description: "Node operating normally" },
              { label: "Warning", color: "var(--status-warning)", description: "Node experiencing issues" },
              { label: "Critical", color: "var(--status-critical)", description: "Node failing or offline" },
            ].map((status) => (
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-3">
              <span className="h-5 w-5 rounded-full border border-primary/40 bg-primary/15" />
              <div>
                <p className="font-medium text-foreground">Consensus Hub</p>
                <p className="text-xs text-muted-foreground">Central coordination point</p>
              </div>
            </div>
          </div>
        </div>
