      ...
      <Card className="glass-card">
        ...
        <CardContent>
          <ChartContainer className="h-80" config={{}}>
            <PieChart>
              <Pie data={severityData} cx="50%" cy="50%" innerRadius={0} outerRadius={120} paddingAngle={2} dataKey="value" nameKey="name" label={(entry) => `${((entry.value / severityData.reduce((sum, item) => sum + item.value, 0)) * 100).toFixed(0)}%`} labelLine={false}>
          ...
          <div className="grid grid-cols-2 gap-3 mt-4">
            {severityData.map((item) => {
              const percentage = ((item.value / severityData.reduce((sum, i) => sum + i.value, 0)) * 100).toFixed(1)
              return (
