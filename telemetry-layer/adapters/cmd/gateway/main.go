...
func loadConfig(cm *utils.ConfigManager) (adapter.Config, error) {
	...
	return adapter.Config{
		...
		SourceType:            cm.GetString("SOURCE_TYPE", "gateway_sensor"),
		...
		IPFIXDiagSampleEvery:  int64(cm.GetInt("IPFIX_DIAG_SAMPLE_EVERY", 500)),
		...
		Kafka: kafka.Config{
			...
			ClientID:      cm.GetString("KAFKA_CLIENT_ID", "telemetry-gateway-adapter"),
