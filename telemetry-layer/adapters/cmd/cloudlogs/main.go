...
func loadConfig(cm *utils.ConfigManager) (adapter.Config, error) {
	...
	provider := strings.ToLower(cm.GetString("CLOUD_PROVIDER", "gcp"))
	sourceType := "gcp_vpc"
	switch provider {
	case "aws":
		sourceType = "aws_vpc"
	...
	return adapter.Config{
		...
		SourceType:            cm.GetString("SOURCE_TYPE", sourceType),
		...
		Kafka: kafka.Config{
			...
			ClientID:      cm.GetString("KAFKA_CLIENT_ID", "telemetry-cloudlogs-adapter"),
