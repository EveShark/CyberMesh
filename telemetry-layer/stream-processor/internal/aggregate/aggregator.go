...
func (a *Aggregator) Add(event model.FlowEvent) []model.FlowAggregate {
	...
	if !ok {
		b = &bucket{
			...
			sourceEventTsMs:     event.SourceEventTsMs,
			telemetryIngestTsMs: event.TelemetryIngestTsMs,
		...
		if event.SourceEventTsMs > 0 && (b.sourceEventTsMs == 0 || event.SourceEventTsMs < b.sourceEventTsMs) {
			b.sourceEventTsMs = event.SourceEventTsMs
		}
		if event.TelemetryIngestTsMs > 0 && (b.telemetryIngestTsMs == 0 || event.TelemetryIngestTsMs < b.telemetryIngestTsMs) {
			b.telemetryIngestTsMs = event.TelemetryIngestTsMs
		}
		if b.verdict == "" {
...
func (a *Aggregator) Flush(now time.Time) []model.FlowAggregate {
	...
	for key, b := range a.buckets {
		...
		out = append(out, model.FlowAggregate{
			...
			SourceEventTsMs:     b.sourceEventTsMs,
			TelemetryIngestTsMs: b.telemetryIngestTsMs,
			SrcIP:               key.key.SrcIP,
			DstIP:               key.key.DstIP,
