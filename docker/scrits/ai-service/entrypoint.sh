...
if [ -n "$AI_SIGNING_KEY_CONTENT" ]; then
    ...
    # Key mounted as file (local dev or alternative K8s setup)
...
if [ "$MODEL_COUNT" -lt 6 ]; then
    echo "ERROR: Expected 6 models (ddos, anomaly, 4 malware variants); only $MODEL_COUNT present"
    echo "Required models: ddos.pkl, anomaly.pkl, malware_api.pkl, malware_flow.pkl, malware_pe_imports.pkl, malware_android.pkl"
    exit 1
fi
...
# ============================================================
# 6. INSTALL RUNTIME PYTHON DEPENDENCIES (GUARD MISSING PACKAGES)
# ============================================================
...
if [ ! -f "$RUNTIME_ENV_PATH" ]; then
    echo "# Generated runtime env - configuration provided via Kubernetes" > "$RUNTIME_ENV_PATH"
fi
