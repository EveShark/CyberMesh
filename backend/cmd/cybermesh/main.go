...
func buildWiringConfig(
	...
	wiringCfg := wiring.Config{
		...
		BacklogCooldown:     cfgMgr.GetDuration("PROPOSER_BACKLOG_COOLDOWN", 250*time.Millisecond),
		PolicyFastCooldown:  cfgMgr.GetDuration("PROPOSER_POLICY_COOLDOWN", 150*time.Millisecond),
		BacklogTxThreshold:  cfgMgr.GetInt("PROPOSER_BACKLOG_TX_THRESHOLD", 100),
		MinMempoolTxs:       cfgMgr.GetInt("BLOCK_MIN_MEMPOOL_TXS", 1),
	...
	if dsn != "" {
		...
		adapterCfg := &cockroach.AdapterConfig{
			...
			Performance: cockroach.AdapterPerformanceConfig{
				...
				OutboxBatchScaleStep:   cfgMgr.GetInt("DB_OUTBOX_UPSERT_BATCH_SCALE_STEP", 16),
				TxStoreFullPayload:     cfgMgr.GetBool("DB_TX_STORE_FULL_PAYLOAD", true),
				TxStoreFullPayloadSet:  true,
				CanaryAutoFallback:     cfgMgr.GetBool("DB_TX_UPSERT_CANARY_FALLBACK_ENABLED", true),
				CanaryAutoFallbackSet:  true,
				CanaryWindowSize:       cfgMgr.GetInt("DB_TX_UPSERT_CANARY_WINDOW_SIZE", 50),
