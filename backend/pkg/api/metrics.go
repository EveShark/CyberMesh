...
func (s *Server) writePrometheusMetrics(w io.Writer) {
	...
	if s.outboxStats != nil {
		...
		if causal, ok := s.outboxStats.GetPolicyAckCausalStats(); ok {
			...
			fmt.Fprintf(w, "# HELP control_policy_ack_correlation_committed_publish_total Total ACK correlations matched to committed publish without requiring outbox replay state.\n")
			fmt.Fprintf(w, "# TYPE control_policy_ack_correlation_committed_publish_total counter\n")
			fmt.Fprintf(w, "control_policy_ack_correlation_committed_publish_total %d\n", causal.CorrelationCommitted)
