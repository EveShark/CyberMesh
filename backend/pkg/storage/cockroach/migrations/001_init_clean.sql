...
CREATE TABLE IF NOT EXISTS consensus_proposals (
    ...
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    INDEX idx_consensus_proposals_height (height),
    INDEX idx_consensus_proposals_view (view_number)
);

COMMENT ON TABLE consensus_proposals IS 'HotStuff proposals stored for crash recovery and replay window restoration';
COMMENT ON COLUMN consensus_proposals.proposal_cbor IS 'CBOR-encoded messages.Proposal payload';
...
CREATE TABLE IF NOT EXISTS consensus_qcs (
    ...
    INDEX idx_consensus_qcs_view (view_number)
);

COMMENT ON TABLE consensus_qcs IS 'HotStuff quorum certificates (aggregated votes)';
COMMENT ON COLUMN consensus_qcs.qc_cbor IS 'CBOR-encoded messages.QC payload';
...
CREATE TABLE IF NOT EXISTS consensus_votes (
    ...
    INDEX idx_consensus_votes_view (view_number),
    INDEX idx_consensus_votes_height (height)
);

COMMENT ON TABLE consensus_votes IS 'Validator votes per view for HotStuff consensus';
COMMENT ON COLUMN consensus_votes.vote_cbor IS 'CBOR-encoded messages.Vote payload';
...
CREATE TABLE IF NOT EXISTS consensus_evidence (
    ...
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    INDEX idx_consensus_evidence_height (height)
);

COMMENT ON TABLE consensus_evidence IS 'Byzantine evidence records for slashing/quarantine';
COMMENT ON COLUMN consensus_evidence.evidence_cbor IS 'CBOR-encoded messages.Evidence payload';

-- =============================================================================
-- CONSENSUS_METADATA TABLE
-- Tracks last committed height/block hash for fast restart
-- =============================================================================
...
COMMENT ON TABLE consensus_metadata IS 'Key-value store for consensus restart metadata';
COMMENT ON COLUMN consensus_metadata.qc_cbor IS 'CBOR-encoded QC for last committed block';
