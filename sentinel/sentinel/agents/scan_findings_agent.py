"""Scan findings agent for scanner outputs (mcp-scan, TruffleHog, Rebuff)."""
...
class ScannerFindingsAgent(BaseAgent):
    """Agent for SCAN_FINDINGS modality (non-rules scanners)."""

    @property
    def name(self) -> str:
        return "scanner_findings_agent"
    ...
    def __call__(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        event = state.get("event")
        if not event:
            return {"errors": ["Missing event for scan findings analysis"], "current_stage": AnalysisStage.DEEP_ANALYSIS}

        try:
            features = ScanFindingsV1.from_dict(event.features)
        except Exception as exc:  # pylint: disable=broad-except
            return {"errors": [f"Invalid scan findings: {exc}"], "current_stage": AnalysisStage.DEEP_ANALYSIS}
        ...
        for item in features.findings:
            ...
            findings.append(Finding(
                ...
                confidence=0.7,
                metadata={"rule_id": item.rule_id, "location": item.location},
            ))
        ...
        updates = {
            ...
            "findings": findings,
            "reasoning_steps": [f"Scanner findings: {len(findings)} findings"],
            "stages_completed": [AnalysisStage.DEEP_ANALYSIS.value],
            "current_stage": AnalysisStage.DEEP_ANALYSIS,
            "errors": errors,
            "metadata": {"scan_findings_time_ms": result.latency_ms},
        }
